...
{
    // Convert buxcoin:// to buxcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("buxcoin://"))
    {
        uri.replace(0, 12, "buxcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Buxcoin.lnk";
}
...
{
    return GetAutostartDir() / "buxcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Buxcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  buxcoin-qt [" + tr("command-line options") + "]                     " + "\n";
